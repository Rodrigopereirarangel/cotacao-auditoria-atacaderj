...
.kpi{cursor:pointer;transition:transform .12s,box-shadow .12s,border-color .12s;position:relative}
.kpi:hover{transform:translateY(-2px);border-color:var(--orange);box-shadow:0 8px 20px rgba(13,40,82,.13)}
.kpi.medir{cursor:default}.kpi.medir:hover{transform:none;border-color:var(--line);box-shadow:0 4px 14px rgba(13,40,82,.06)}
.kpi .hint{font-size:9.5px;color:var(--orange);font-weight:700;margin-top:6px;letter-spacing:.3px}
.kpi.medir .hint{display:none}
.modal{position:fixed;inset:0;background:rgba(13,40,82,.55);display:none;align-items:center;justify-content:center;z-index:5000;padding:20px}
.modal.show{display:flex}
.modal .box{background:var(--surface);border-radius:14px;padding:20px 22px;width:min(760px,94vw);box-shadow:0 24px 70px rgba(0,0,0,.35)}
.modal .box h3{font-size:17px;color:var(--navy);margin:0 30px 2px 0}
.modal .box .msub{font-size:12px;color:var(--dim);margin-bottom:14px}
.modal .x{position:absolute;margin:-4px 0 0 0;cursor:pointer;font-size:24px;color:var(--dim);line-height:1;border:none;background:none;float:right}
.modal .box{position:relative}.modal .x{position:absolute;top:16px;right:18px}
...
<div class="modal" id="modal"><div class="box"><button class="x" id="mX">&times;</button><h3 id="mTit"></h3><div class="msub" id="mSub"></div><canvas id="cModal"></canvas></div></div>
...
(function(){
  ...
  var k=document.getElementById('kpis');function kpi(l,v,medir,key){var d=document.createElement('div');d.className='kpi'+(medir?' medir':'');d.innerHTML='<div class="lbl">'+l+'</div><div class="val">'+v+'</div><div class="hint">ver evolucao ▸</div>';if(key&&!medir)d.onclick=function(){abrirModal(key);};k.appendChild(d);}
  if(u){
    kpi('Qualidade (P/R)', f(u.precisao,1)+'<small>% / '+f(u.recall,1)+'%</small>', false, 'qualidade');
    kpi('Itens sem match', US?(f(US.sem_match_taxa*100,1)+'<small>%</small>'):'a medir', !US, 'sem_match');
    kpi('Tempo da cotacao', US?seg(US.tempo_medio_cotacao_ms):'a medir', !US, 'tempo');
    kpi('Tempo medio/item', US?(Math.round(US.tempo_medio_item_ms)+'ms'):'a medir', !US, 'tempo_item');
    kpi('Tokens / cotacao', f(u.tokens), false, 'tokens');
    kpi('Req / cotacao', f(u.req,1), false, 'req');
    kpi('Cache-hit', f((u.cache||0)*100,1)+'<small>%</small>', false, 'cache');
  }
  kpi('Apelidos', D.biblioteca.apelidos, false, 'apelidos');
  ...
  // --- modais de evolucao (clique nos KPIs) ---
  function _serie(arr,label,color){return {label:label,data:arr,borderColor:color,backgroundColor:color};}
  var _am=US?(US.amostras||[]):[];var _amL=_am.map(function(a,i){return a.d?a.d.slice(5,10):('#'+(i+1));});
  var MET={
    qualidade:{t:'Qualidade — precisao / recall',s:'por rodada',f:function(){return {type:'line',data:{labels:labels,datasets:[_serie(R.map(function(r){return r.precisao;}),'Precisao %',navy),_serie(R.map(function(r){return r.recall;}),'Recall %',orange)]}};}},
    sem_match:{t:'Itens sem match / errados',s:US?'% por cotacao (uso real)':'por rodada',f:function(){return US?{type:'line',data:{labels:_amL,datasets:[_serie(_am.map(function(a){return a.itens?+(a.sem/a.itens*100).toFixed(1):null;}),'Sem match %',red)]}}:{type:'line',data:{labels:labels,datasets:[_serie(R.map(function(r){return r.sem_match;}),'Sem match',red),_serie(R.map(function(r){return r.errados;}),'Errados',orange)]}};}},
    tempo:{t:'Tempo da cotacao',s:US?'segundos por cotacao (uso real)':'por rodada',f:function(){return US?{type:'line',data:{labels:_amL,datasets:[_serie(_am.map(function(a){return +(a.t/1000).toFixed(1);}),'Cotacao (s)',navy)]}}:{type:'line',data:{labels:labels,datasets:[_serie(R.map(function(r){return r.tempo_total_ms==null?null:r.tempo_total_ms/1000;}),'Cotacao (s)',navy)]}};}},
    tempo_item:{t:'Tempo medio por item',s:US?'ms por item (uso real)':'por rodada',f:function(){return US?{type:'line',data:{labels:_amL,datasets:[_serie(_am.map(function(a){return a.itens?Math.round(a.t/a.itens):null;}),'Por item (ms)',orange)]}}:{type:'line',data:{labels:labels,datasets:[_serie(R.map(function(r){return r.tempo_item_ms;}),'Por item (ms)',orange)]}};}},
    tokens:{t:'Tokens por cotacao',s:'por rodada',f:function(){return {type:'line',data:{labels:labels,datasets:[_serie(R.map(function(r){return r.tokens;}),'Tokens/cotacao',navy)]}};}},
    req:{t:'Requisicoes por cotacao',s:'por rodada',f:function(){return {type:'line',data:{labels:labels,datasets:[_serie(R.map(function(r){return r.req;}),'Req/cotacao',navy)]}};}},
    cache:{t:'Cache-hit',s:'% por rodada',f:function(){return {type:'line',data:{labels:labels,datasets:[_serie(R.map(function(r){return (r.cache||0)*100;}),'Cache-hit %',green)]}};}},
    apelidos:{t:'Biblioteca (estado atual)',s:'apelidos / buscas / ausentes / correcoes / motivos',f:function(){return {type:'bar',data:{labels:['Apelidos','Buscas','Ausentes','Correcoes','Motivos IA'],datasets:[{label:'itens',data:[D.biblioteca.apelidos,D.biblioteca.buscas,D.biblioteca.ausentes,D.biblioteca.correcoes,D.biblioteca.apelido_motivos],backgroundColor:[navy,'#2f74c4',orange,green,'#9a5a00']}]},options:{plugins:{legend:{display:false}}}};}},
  };
  var _mc=null;
  function abrirModal(key){var m=MET[key];if(!m)return;document.getElementById('mTit').textContent=m.t;document.getElementById('mSub').textContent=m.s||'';if(_mc){_mc.destroy();_mc=null;}var cfg=m.f();if(!cfg.options)cfg.options={responsive:true,plugins:{legend:{position:'bottom'}},tension:.25};_mc=new Chart(document.getElementById('cModal'),cfg);document.getElementById('modal').classList.add('show');}
  document.getElementById('mX').onclick=function(){document.getElementById('modal').classList.remove('show');};
